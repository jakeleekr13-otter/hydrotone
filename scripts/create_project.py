...
for scope in ['project', 'app', 'tests', 'uitests']:
    ...
    for config in ['Debug', 'Release']:
        ...
        if scope == 'project':
            ...
            settings.update({'PRODUCT_NAME':'$(TARGET_NAME)','PRODUCT_BUNDLE_IDENTIFIER':{'app':'com.hydrotone.app','tests':'com.hydrotone.tests','uitests':'com.hydrotone.uitests'}[scope], 'GENERATE_INFOPLIST_FILE':'YES','CODE_SIGN_STYLE':'Automatic'})
        if scope == 'app':
...
add('project',f'isa = PBXProject; attributes = {{ BuildIndependentTargetsInParallel = YES; LastUpgradeCheck = 2700; }}; buildConfigurationList = {configs["project"]}; compatibilityVersion = "Xcode 16.0"; developmentRegion = en; knownRegions = (en,Base); mainGroup = {main}; productRefGroup = {pg}; projectDirPath = ""; projectRoot = ""; targets = ({",".join(targets)},); preferredProjectObjectVersion = 77;')
